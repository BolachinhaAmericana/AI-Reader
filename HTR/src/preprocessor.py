...
class Preprocessor:
    ...
    def process_img(self, img: np.ndarray) -> np.ndarray:
        ...
        if self.data_augmentation:
            ...
            h, w = img.shape
            f = min(wt / w, ht / h)
            fx = f * np.random.uniform(0.75, 1)
            fy = f * np.random.uniform(0.75, 1)

            # random position around center
            txc = (wt - w * fx) / 2
            tyc = (ht - h * fy) / 2
            freedom_x = max((wt - fx * w) / 2, 0) * 0
            ...
            if random.random() < 0.3:
                img = np.clip(img + (np.random.random(img.shape) - 0.7) * random.randint(10, 20), 120, 255)
            #if random.random() < 0.1:
            #    img = 255 - img
...
def main():
    ...
    plt.subplot(122)
    plt.imshow(cv2.transpose(img_aug) + 0.2, cmap='gray')
    plt.show()
